...
from .models import PullRequest, Moderator
...
def handle_pull_request(payload: dict):
    ...
    if issue:=payload.get('issue'):
        ...
        if pr['user']['id'] != account_id: return
    match payload['action']:
        case 'closed':
            PullRequest.objects.get(number=payload['number']).delete()
            word: str = re.search(r"'(?P<word>.+)'\s", payload['issue']['title']).group('word')
...
def make_pull_request():
    ...
    """    
    words: list[Word] = Word.objects.annotate(num_usertranslation=Count('usertranslation')).filter(num_usertranslation__gt=3, pullrequest=None)
    if not words: return
    ...
    for word in words:
        try:
            ...
            pass
        dictionary_repo.create_git_ref(f'refs/heads/Torjoman-{word.word}', source_branch.commit.sha)
        ...
        pull = dictionary_repo.create_pull(
            ...
            body=body,
            head=f"Torjoman-{word.word}", base='main'
        )
